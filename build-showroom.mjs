 ...
 *
 * No per-repo setup needed. To keep a repo off the rail, add its name to EXCLUDE.
 *
...
// Repos to keep off the rail. The showroom itself, plus anything unfinished.
const EXCLUDE = new Set([
  'showroom',
  `${USER}.github.io`
]);

// Optional: nicer card titles than the repo name. Repo name on the left.
const TITLES = {
...
// Title, in order of preference: the TITLES map above, then the part of the repo
// description before a dash or pipe, then a tidied-up repo name.
...
// Card copy: whatever's left of the description after the title is taken out.
...
async function main() {
  ...
  // Don't overwrite a good manifest with an empty one.
  if (!pieces.length) {
